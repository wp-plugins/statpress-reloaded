...
####################################################################################################
	...
	foreach ($qry as $rk) {
		print "<tr><td>".irihdate($rk->date)."</td><td>".$rk->time."</td><td><a href='".$rk->referrer."'>".$rk->search."</a></td><td>".$rk->searchengine."</td><td><a href='".get_bloginfo('url') . ((!permalinksEnabled()) ? "/?" : "" ) . $rk->urlrequested."'>". __('page viewed','statpress'). "</a></td></tr>\n";
	}
	print "</table></div>";
	...
	foreach ($qry as $rk) {
		print "<tr><td>".irihdate($rk->date)."</td><td>".$rk->time."</td><td><a href='".$rk->referrer."'>".iri_StatPress_Abbrevia($rk->referrer,80)."</a></td><td><a href='".get_bloginfo('url') . ((!permalinksEnabled()) ? "/?" : "" ) . $rk->urlrequested."'>". __('page viewed','statpress'). "</a></td></tr>\n";
	}
	print "</table></div>";
